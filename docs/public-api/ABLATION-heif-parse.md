# ABLATION-heif-parse — Conservative Public-API Review

**Date:** 2026-06-10
**Snapshot commit:** 4c23c48883e3 (main@origin)
**Snapshot file:** docs/public-api/heif-parse.txt (598 default items / 692 all-features items)
**Grep template:** `grep -rn "<SYMBOL>" /home/lilith/work/ --include="*.rs" 2>/dev/null | grep -v "/zen/heif-parse/" | grep -v "target/" | grep -v ".jj/"`
...
**Proposed action A:** Add `#[deprecated(since = "1.5.0", note = "Use `DecodeConfig::lenient()` with `AvifParser` instead")]` to `ParseOptions` at `/home/lilith/work/zen/heif-parse/src/lib.rs:728` (before the `#[derive(Debug, Clone, Copy)]` line).
...
- `/home/lilith/work/zen/zenavif/Cargo.toml` — `heif-parse/zencodec`
- `/home/lilith/work/zen/gainmap-spec-status/tools/corpus-test/Cargo.toml` — `heif-parse = { features = ["zencodec"] }`
